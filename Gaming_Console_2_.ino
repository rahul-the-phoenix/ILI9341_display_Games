...
void showStartScreenSnake() {
  ...
  for(int i=0;i<3;i++) tft.drawRect(i,i,screenWidth-i*2,screenHeight-i*2,(i==0)?COL_GOLD:(i==1)?(uint16_t)0x3186:COL_BG);
  ...
  tft.setTextColor(COL_GOLD); tft.setCursor(30,152); tft.print("SELECT   : Pause / Resume");
  tft.setTextColor(0xAFE5); tft.setCursor(30,166); tft.print("B        : Back to menu");
  tft.setTextColor(0x3186); tft.setCursor(30,180);
  char hsbuf[24]; sprintf(hsbuf,"Best Score : %d",highScoreSnake); tft.print(hsbuf);
...
void showSnakePauseScreen(bool pause) {
  if(pause) { tft.fillRect(60,80,200,60,0x001F); tft.drawRect(60,80,200,60,COL_SCORE); tft.setCursor(120,105); tft.setTextColor(COL_SCORE); tft.setTextSize(3); tft.print("PAUSED"); tft.setTextSize(2); }
  else { tft.fillRect(60,80,200,60,COL_BG); drawSnakeFood(false); if(bonusActive) drawBonusFood(false); }
}
void spawnFood() {
  drawSnakeFood(true);
  int maxGX=(tft.width()-20)/SNAKE_GRID, maxGY=(tft.height()-50)/SNAKE_GRID;
  bool ok;
  do { ok=true; food.x=random(2,maxGX-2)*SNAKE_GRID+10; food.y=random(4,maxGY-2)*SNAKE_GRID+40;
    for(int i=0;i<snakeLen;i++) if(abs(snake[i].x*SNAKE_GRID+10-food.x)<SNAKE_GRID&&abs(snake[i].y*SNAKE_GRID+40-food.y)<SNAKE_GRID){ok=false;break;}
  } while(!ok);
  drawSnakeFood(false);
}
void spawnBonusFood() {
  if(bonusActive) return; bonusActive=true; bonusStartTime=millis();
  int maxGX=(tft.width()-20)/SNAKE_GRID, maxGY=(tft.height()-50)/SNAKE_GRID;
  bool ok;
  do { ok=true; bonusFood.x=random(2,maxGX-2)*SNAKE_GRID+10; bonusFood.y=random(4,maxGY-2)*SNAKE_GRID+40;
    for(int i=0;i<snakeLen;i++) if(abs(snake[i].x*SNAKE_GRID+10-bonusFood.x)<SNAKE_GRID&&abs(snake[i].y*SNAKE_GRID+40-bonusFood.y)<SNAKE_GRID){ok=false;break;}
...
void drawSnakePart(int index, bool erase) {
  ...
  if(index==0){tft.fillCircle(x+3,y+3,2,COL_BG);tft.fillCircle(x+SNAKE_GRID-4,y+3,2,COL_BG);tft.fillCircle(x+3,y+3,1,COL_SCORE);tft.fillCircle(x+SNAKE_GRID-4,y+3,1,COL_SCORE);}
}
void drawSnakeFood(bool erase) {
  if(erase){tft.fillCircle(food.x,food.y,8,COL_BG);return;}
  tft.fillCircle(food.x,food.y,7,COL_FOOD); tft.fillCircle(food.x,food.y,4,COL_SCORE); tft.fillCircle(food.x,food.y,2,0x001F);
}
void drawBonusFood(bool erase) {
  ...
  for(int i=0;i<8;i++){float a=i*45*3.14159/180;tft.fillCircle(bonusFood.x+cos(a)*12,bonusFood.y+sin(a)*12,2,COL_GOLD);}
}
void showSnakeGameOver() {
  if(score>highScoreSnake){highScoreSnake=score;menuHighScores[0]=highScoreSnake;}
  tft.fillScreen(COL_BG); tft.drawRect(5,5,tft.width()-10,tft.height()-10,COL_GOLD); tft.drawRect(8,8,tft.width()-16,tft.height()-16,COL_GOLD);
  tft.setTextSize(3); tft.setTextColor(COL_SNAKE_HEAD); tft.setCursor(tft.width()/2-80,40); tft.print("GAME OVER");
  tft.drawFastHLine(30,85,tft.width()-60,COL_SCORE);
  tft.fillRoundRect(20,100,tft.width()-40,90,10,0x1082);
  tft.setTextSize(2); tft.setTextColor(COL_SCORE); tft.setCursor(40,120); tft.print("YOUR SCORE : "); tft.setTextColor(COL_GOLD); tft.print(score);
  tft.setTextColor(COL_SCORE); tft.setCursor(40,150); tft.print("HIGH SCORE : "); tft.setTextColor(COL_FOOD); tft.print(highScoreSnake);
...
void resetSnakeGame() {
  tft.fillScreen(COL_BG); snakeLen=5; score=0; foodCounter=0; moveDelay=150;
  ...
  dirX=1; dirY=0; spawnFood(); updateSnakeScoreDisplay();
...
void runSnakeGame() {
  ...
  if(!gameOverSnake&&!isPausedSnake){
    ...
    if(millis()-lastPulseTime>150){lastPulseTime=millis();pulseState++;if(bonusActive){if(millis()-bonusStartTime>4000){drawBonusFood(true);bonusActive=false;}else drawBonusFood(false);}drawSnakeFood(false);updateSnakeScoreDisplay();}
    if(millis()-lastMoveTime>moveDelay){
      ...
      Point nh={nx,ny};
      for(int i=0;i<snakeLen;i++) if(nh.x==snake[i].x&&nh.y==snake[i].y){gameOverSnake=true;showSnakeGameOver();return;}
      bool ate=false;
      int hpx=nh.x*SNAKE_GRID+10+SNAKE_GRID/2,hpy=nh.y*SNAKE_GRID+40+SNAKE_GRID/2;
      if(abs(hpx-food.x)<SNAKE_GRID&&abs(hpy-food.y)<SNAKE_GRID){score+=10;snakeLen++;foodCounter++;ate=true;if(moveDelay>50)moveDelay-=2;spawnFood();if(foodCounter>=5&&!bonusActive){spawnBonusFood();foodCounter=0;}}
      else if(bonusActive&&abs(hpx-bonusFood.x)<SNAKE_GRID+2&&abs(hpy-bonusFood.y)<SNAKE_GRID+2){unsigned long t=(millis()-bonusStartTime)/1000;int dyn=80-(t*10);if(dyn<10)dyn=10;score+=dyn;snakeLen++;bonusActive=false;ate=true;drawBonusFood(true);}
